...
# 【修正】オブジェクトを作成してから個別にtranslateを実行
arm_l_sphere = Part.makeSphere(limb_r)
arm_l_sphere.translate(pos_arm_l)
arm_l = arm_l_sphere.fuse(make_fingers_horizontal_fixed(pos_arm_l))

arm_r_sphere = Part.makeSphere(limb_r)
arm_r_sphere.translate(pos_arm_r)
arm_r = arm_r_sphere.fuse(make_fingers_horizontal_fixed(pos_arm_r))

leg_l_sphere = Part.makeSphere(limb_r)
leg_l_sphere.translate(pos_leg_l)
leg_l = leg_l_sphere.fuse(make_fingers_horizontal_fixed(pos_leg_l))

leg_r_sphere = Part.makeSphere(limb_r)
leg_r_sphere.translate(pos_leg_r)
leg_r = leg_r_sphere.fuse(make_fingers_horizontal_fixed(pos_leg_r))
...
slice_box_front.translate(App.Base.Vector(-50, -25, -35))

slice_box_back = Part.makeBox(50, 50, 60)
slice_box_back.translate(App.Base.Vector(0, -25, -35))
...
# --- 4. FreeCADの画面に出力 ---
obj_front = doc.addObject("Part::Feature", "FlyingMocapy_FRONT")
obj_front.Shape = front_part

obj_back = doc.addObject("Part::Feature", "FlyingMocapy_BACK")
obj_back.Shape = back_part

doc.recompute()

# 【改善】GUI環境のみで実行されるよう安全策を追加
if App.GuiUp:
    try:
